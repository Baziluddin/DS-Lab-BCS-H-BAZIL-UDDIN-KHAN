...
class AvlTree
{
    ...
    Node * InsertNode(Node * Root,Node * node)
    {
        ...
        {
            cout << " Right Roation (LL) with Balance Factor = " << bf  << endl;
            return RightRotaion(Root);
        }
       
        else if (bf < -1 && node->Data > Root->RightChild->Data)
        {
            cout << " Left Roation (RR) with Balance Factor = " << bf  << endl;
    ...
    Node * Delete(Node * Root , Node * node)
    {
        ...
        int bf = GetBf(Root);
        cout << "Balance Factor after deletion at node " << Root->Data << " = " << bf << endl;
